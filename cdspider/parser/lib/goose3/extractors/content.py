...
KNOWN_CONTENT_TAGS = [
    {'tag': 'article', 'content': 'text'},
]

KNOWN_CONTENT_TAGS_BY_DOMAIN = {
}

KNOWN_CONTENT_PATTERN_BY_DOMAIN = {
    'toutiao.com': [
        'content:\s+(?:\'|")([^\'"]+)(?:\'|").*'
    ]
}
...
class ContentExtractor(BaseExtractor):
    ...
    def calculate_best_node(self):
        ...
        try:
            ...
            for node in nodes_to_check:
                ...
                high_link_density = self.is_highlink_density(node, word_stats.get_word_count())

                if self.stopwords_class.__name__ == 'StopWordsChinese':
                    if word_stats.get_word_count() > 3 and not high_link_density:
                        nodes_with_text.append(node)
                elif word_stats.get_stopword_count() > 2 and not high_link_density:
                    nodes_with_text.append(node)
    ...
    def is_highlink_density(self, element, cnt = 0):
        """
        ...
        number_of_link_words = float(self.stopwords_class(language=self.get_language()).get_word_count(link_text).get_word_count())
