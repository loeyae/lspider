...
class ParsingCandidate(object):
    ...
    def __init__(self, url_string, link_hash, encoding=None):
        self.url = url_string
...
class RawHelper(object):
    @classmethod
    def get_parsing_candidate(cls, url, raw_html, encoding=None):
        if isinstance(raw_html, str):
            raw_html = raw_html.encode('utf-8')
        link_hash = '%s.%s' % (hashlib.md5(raw_html).hexdigest(), time.time())
        return ParsingCandidate(url, link_hash, encoding)
