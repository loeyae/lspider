...
class Parser(object):
    ...
    def parser_time(html, now = False):
        ...
        rule12 = '[\>\040;](?:(?:本文)?发布于|发表于|发布(?:时间|日期)|日期|时间)\s*(?:\:|：)?\040*'+ rule1
        rule13 = rule1 +'\s*发布'
        rule14 = '[\040\>;]((?:(?:(?:(?:20)?[012]\d)|(?:19)?[9]\d)-\d{1,2}-\d{1,2}(?:\D*\d{1,2}:\d{2}(?::\d{2})?)?)|(?:(?:(?:(?:20)?[012]\d)|(?:19)?[9]\d)\/\d{1,2}\/\d{1,2}(?:\D*\d{1,2}:\d{2}(?::\d{2})?)?))(?:\040|&nbsp;)+\<'
