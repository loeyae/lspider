...
class ResultDB(Mongo, BaseResultDB, SplitTableMixin):
    ...
    def insert(self, obj = {}):
        ...
        obj['rid'] = BaseResultDB.build_id(obj['createtime'], id)
        obj.setdefault('status', self.RESULT_STATUS_INIT)
        obj.setdefault('urlid', 0)
        obj.setdefault('atid', 0)
        obj.setdefault('kwid', 0)
        super(ResultDB, self).insert(setting=obj, table=table)
        return obj['rid']

    def update(self, id, obj = {}):
        table = self._table_name(id)
        obj['updatetime'] = int(time.time())
        return super(ResultDB, self).update(setting=obj, where={"rid": id}, table=table)
    ...
    def _get_collection(self, createtime):
        ...
        name = super(ResultDB, self)._collection_name(suffix)
    ...
    def _create_collection(self, table):
        ...
        if not 's_c' in indexes:
            collection.create_index([('siteid', 1), ('crawl_id', 1)], name='s_c')
        if not 'p_s_u' in indexes:
            collection.create_index([('projectid', 1), ('siteid', 1), ('urlid', 1)], name='p_s_u')
