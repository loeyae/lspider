...
class ProjectsDB(Mongo, BaseProjectsDB):
    ...
    def active(self, id):
        return super(ProjectsDB, self).update(setting={"status": self.STATUS_ACTIVE}, where={"uuid": int(id)},
                                              multi=False)

    def disable(self, id):
        return super(ProjectsDB, self).update(setting={"status": self.STATUS_INIT}, where={"uuid": int(id)},
                                              multi=False)

    def delete(self, id):
        return super(ProjectsDB, self).update(setting={"status": self.STATUS_DELETED}, where={"uuid": int(id)},
                                              multi=False)
