...
class SpiderTaskDB(Mongo, BaseTaskDB, SplitTableMixin):
    ...
    def get_active_count(self, mode, where={}):
        table = self._table_name(mode)
        now = int(time.time())
    ...
    def get_plan_list(self, mode, id, plantime=None, where={}, select=None, **kwargs):
        ...
        where = self._build_where(where)
        _where = {'$and': [
            {'status': self.STATUS_ACTIVE}, {'plantime': {'$lte': plantime}},
            {'$or': [{'expire': 0}, {'expire': {'$gt': now}}]},
            {"uuid": {"$gt": id}}
        ]}
        for k, v in where.items():
            _where['$and'].extend([{k: v}])
    ...
    def _create_collection(self, table):
        ...
        if 'u_k_p_r' not in indexes:
            collection.create_index([("tid", pymongo.ASCENDING), ("uid", pymongo.ASCENDING), ("kid", pymongo.ASCENDING), ("rid", pymongo.ASCENDING), ("parentid", pymongo.ASCENDING)], unique=True, name='u_k_p_r')
